package com.weiwan.box.module.infra.service.codegen.inner;
...
import com.weiwan.box.framework.apilog.core.annotation.ApiAccessLog;
import com.weiwan.box.framework.apilog.core.enums.OperateTypeEnum;
...
import com.weiwan.box.framework.excel.core.annotations.DictFormat;
import com.weiwan.box.framework.excel.core.convert.DictConvert;
import com.weiwan.box.framework.excel.core.util.ExcelUtils;
import com.weiwan.box.framework.mybatis.core.dataobject.BaseDO;
...
import com.weiwan.box.module.infra.enums.codegen.CodegenVOTypeEnum;
...
public class CodegenEngine {
    ...
    /**
     * 是否为 easyboot-cloud 项目，用于解决 Boot 和 Cloud 的 api 模块兼容性问题
     *
     * true  - 需要有 easyboot-module-xxx-api 模块
    ...
    public CodegenEngine() {
        ...
        this.cloudEnable = ClassUtils.isPresent("com.weiwan.box.module.infra.framework.rpc.config.RpcConfiguration",
    ...
    private Map<String, String> getTemplates(Integer frontType) {
        ...
        if (Boolean.FALSE.equals(cloudEnable)) {
            SERVER_TEMPLATES.forEach((templatePath, filePath) -> {
                filePath = StrUtil.replace(filePath, "/easyboot-module-${table.moduleName}-api", "");
                filePath = StrUtil.replace(filePath, "/easyboot-module-${table.moduleName}-server", "");
    ...
    private static String javaModuleFilePath(String path, String module, String src) {
        return "easyboot-module-${table.moduleName}/" + // 顶级模块
                "easyboot-module-${table.moduleName}-" + module + "/" + // 子模块
                "src/" + src + "/java/${basePackage}/module/${table.moduleName}/" + path + ".java";
    }

    private static String mapperXmlFilePath() {
        return "easyboot-module-${table.moduleName}/" + // 顶级模块
                "easyboot-module-${table.moduleName}-server/" + // 子模块
    ...
    private static String vueFilePath(String path) {
        return "easyboot-ui-${sceneEnum.basePackage}-vue2/" + // 顶级目录
                "src/" + path;
    }
    ...
    private static String vue3FilePath(String path) {
        return "easyboot-ui-${sceneEnum.basePackage}-vue3/" + // 顶级目录
                "src/" + path;
    }

    private static String vue3VbenFilePath(String path) {
        return "easyboot-ui-${sceneEnum.basePackage}-vben/" + // 顶级目录
                "src/" + path;
    }
