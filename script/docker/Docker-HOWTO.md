...
依赖：docker compose v2，删除`name: easyboot-system`，降低`version`版本为`3.3`以下，支持`docker-compose`。
...
```shell
# 创建maven缓存volume
docker volume create --name easyboot-maven-repo

docker run -it --rm --name easyboot-maven \
    -v easyboot-maven-repo:/root/.m2 \
